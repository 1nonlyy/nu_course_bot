...
async def check_one_course(
    ...
    *,
    ...
    """
    Scrape one course, update snapshots and notification state, send pushes as needed.
    Raises on unexpected errors so the caller can aggregate with asyncio.gather.
    """
    ...
    try:
        ...
        results = await asyncio.gather(
            *[
                ...
                for code, ids in items
            ],
            return_exceptions=True,
        )
        for (course_code, user_ids), result in zip(items, results):
            if isinstance(result, Exception):
                logger.error(
                    "poll_catalog course check failed",
                    course_code=course_code,
                    user_count=len(user_ids),
                    error=str(result),
                    exc_info=result,
